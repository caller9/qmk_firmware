...
const rgblight_segment_t PROGMEM my_capslock_layer[] = RGBLIGHT_LAYER_SEGMENTS({0, 3, HSV_RED});   // 3
...
static void inline show_default_img(void) {
    ...
    my_image = qp_load_image_mem(gfx_xiongmaotou_st7735s);
    if (my_image != NULL) {
...
extern bool should_multi_qp_task(void) {
    return !is_in_flash_writing && !use_default_ui;
}
#endif

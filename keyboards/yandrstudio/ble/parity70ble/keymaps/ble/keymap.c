...
#ifdef FACTORY_TEST
        KC_LCTL, KC_LGUI, KC_LALT,                            FT_BT_1,                   FT_BT_2, FT_BT_3, KC_RALT, KC_RCTL,            KC_LEFT, KC_DOWN, KC_RIGHT),
#else
        KC_LCTL, KC_LGUI, KC_LALT,                            KC_SPC,                    KC_RGUI, KC_RALT, MO(1),   KC_RCTL,           KC_LEFT, KC_DOWN, KC_RIGHT),
#endif
    LAYOUT(
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        ...
        case KC_SPC: {
            dprintf("ADC VAL: %d\n", debug_ble_info_helper());
            char bat_str[12] = {0};
            snprintf(bat_str, sizeof(bat_str), "%d", debug_ble_info_helper());
            bat_str[5]='\0';
            send_string(bat_str);
            break;
        }
        ...
        case KC_BTSH: {
            if (record->event.pressed) {
                ...
                if (bat_l_t <= 10) {
                    rgblight_blink_layer_repeat(11, 1000, 1);
                } else if (bat_l_t <= 30) {
                    rgblight_blink_layer_repeat(12, 1000, 1);
                } else if (bat_l_t <= 50) {
                    rgblight_blink_layer_repeat(13, 1000, 1);
                } else if (bat_l_t <= 70) {
                    rgblight_blink_layer_repeat(14, 1000, 1);
                } else {
                    rgblight_blink_layer_repeat(15, 1000, 1);
                }
            }
        }
        return false;

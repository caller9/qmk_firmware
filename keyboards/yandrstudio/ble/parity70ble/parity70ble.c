...
const rgblight_segment_t PROGMEM bt_conn4[] = RGBLIGHT_LAYER_SEGMENTS( {3, 1, HSV_GREEN} );      // 3
const rgblight_segment_t PROGMEM bt_pair1[] = RGBLIGHT_LAYER_SEGMENTS( {0, 1, HSV_BLUE} );       // 4
const rgblight_segment_t PROGMEM bt_pair2[] = RGBLIGHT_LAYER_SEGMENTS( {1, 1, HSV_BLUE} );       // 5
const rgblight_segment_t PROGMEM bt_pair3[] = RGBLIGHT_LAYER_SEGMENTS( {2, 1, HSV_BLUE} );       // 6
const rgblight_segment_t PROGMEM bt_pair4[] = RGBLIGHT_LAYER_SEGMENTS( {3, 1, HSV_BLUE} );       // 7
const rgblight_segment_t PROGMEM usb_pair[] = RGBLIGHT_LAYER_SEGMENTS( {0, 4, HSV_BLUE});        // 8
const rgblight_segment_t PROGMEM usb_conn[] = RGBLIGHT_LAYER_SEGMENTS( {0, 4, HSV_GREEN});       // 9
const rgblight_segment_t PROGMEM my_capslock_layer[] = RGBLIGHT_LAYER_SEGMENTS({0, 4, HSV_RED}); // 10
const rgblight_segment_t PROGMEM bat_10_layer[] = RGBLIGHT_LAYER_SEGMENTS({0, 4, HSV_RED});      // 11
const rgblight_segment_t PROGMEM bat_30_layer[] = RGBLIGHT_LAYER_SEGMENTS({0, 1, HSV_CYAN});     // 12
const rgblight_segment_t PROGMEM bat_50_layer[] = RGBLIGHT_LAYER_SEGMENTS({0, 2, HSV_CYAN});     // 13
const rgblight_segment_t PROGMEM bat_70_layer[] = RGBLIGHT_LAYER_SEGMENTS({0, 3, HSV_CYAN});     // 14
const rgblight_segment_t PROGMEM bat_100_layer[] = RGBLIGHT_LAYER_SEGMENTS({0, 4, HSV_CYAN});    // 15
...
static void inline show_default_img(void) {
    ...
    my_image = qp_load_image_mem_force(gfx_mxrskey);
    if (my_image != NULL) {
        my_anim = qp_drawimage(lcd, 11, 0, my_image);
    } else {
...
void keyboard_post_init_kb(void) {
    ...
    if (my_image != NULL) {
        my_anim = qp_animate(lcd, 11, 0, my_image);
...
#if defined(RGBLIGHT_ENABLE) && !defined(FACTORY_TEST)

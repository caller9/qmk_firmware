...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    LAYOUT(
        ...
        KC_TRNS, BL_SW_0, BL_SW_1, BL_SW_2, BL_SW_3, ESB_TOG,  KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS,
...
static uint8_t conn_mode = 0;
static uint8_t host_idx = 0;
static bool has_in_pairing =false;
static uint8_t ble_op_type = 0;
static virtual_timer_t switch_connection_mode_timer;
...
static void switch_connection_mode_timer_cb(void *arg) {
    switch_output_driver(conn_mode);
    ble_channle_update(true);
}
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        case BLE_TOG ... ESB_TOG:
            if (record->event.pressed) {
                conn_mode = keycode - BLE_TOG;
                chVTSet(&switch_connection_mode_timer, TIME_MS2I(1000), (vtfunc_t)switch_connection_mode_timer_cb, NULL);
            } else {
                chVTReset(&switch_connection_mode_timer);
            }
            return false;
        // case USB_TOG:
        //     if (record->event.pressed) {
